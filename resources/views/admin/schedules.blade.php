    ...
    <div class="glass-card rounded-2xl p-6">
        ...
        <form action="{{ route('admin.schedules.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="academic_map_id" required class="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:border-blue-500">
                    <option value="">-- Pilih Pemetaan --</option>
                    @foreach($academicMaps as $map)
                    <option value="{{ $map->id }}">{{ $map->class->name ?? '-' }} | {{ $map->subject->name ?? '-' }} | {{ $map->teacher->name ?? '-' }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Hari</label>
                <select name="day" required class="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:border-blue-500">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="time" name="start_time" required class="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Selesai</label>
                    <input type="time" name="end_time" required class="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:border-blue-500">
                </div>
            </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        <td class="px-5 py-4">
                            <span class="px-2.5 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700">{{ $schedule->day }}</span>
                        </td>
                        <td class="px-5 py-4 text-sm text-slate-600 font-medium">
                            {{ \Carbon\Carbon::parse($schedule->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($schedule->end_time)->format('H:i') }}
                        </td>
                        <td class="px-5 py-4 text-sm font-semibold text-slate-800">{{ $schedule->academicMap->class->name ?? '-' }}</td>
                        <td class="px-5 py-4 text-sm text-slate-600">{{ $schedule->academicMap->subject->name ?? '-' }}</td>
                        <td class="px-5 py-4 text-sm text-slate-600">{{ $schedule->academicMap->teacher->name ?? '-' }}</td>
                        <td class="px-5 py-4">
                            <form action="{{ route('admin.schedules.destroy', $schedule) }}" method="POST" onsubmit="return confirm('Hapus jadwal?')">
                                @csrf @method('DELETE')
