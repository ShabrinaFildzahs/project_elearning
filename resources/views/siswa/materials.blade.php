...
<div class="space-y-6">
    ...
    <div class="glass-card p-5 rounded-2xl flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">Total <span class="font-semibold text-blue-600">{{ $materials->total() }}</span> materi tersedia</p>
        </div>
    ...
    @if($materials->isEmpty())
        <div class="glass-card p-16 rounded-2xl text-center">
            ...
            <p class="text-slate-500 text-sm mt-2">Guru belum mengupload materi. Pantau terus!</p>
        </div>
    @else
                ...
                <div class="p-6">
                    <div class="flex items-start space-x-4 mb-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="font-bold text-slate-800 leading-snug truncate">{{ $material->title }}</h4>
                            <p class="text-xs text-slate-500 mt-1">
                                {{ $material->academicMap->subject->name ?? '-' }} — 
                                <span class="font-medium">Kelas {{ $material->academicMap->class->name ?? '-' }}</span>
                            </p>
                        </div>
                    </div>
                    @if($material->description)
                        <p class="text-sm text-slate-600 leading-relaxed line-clamp-2 mb-4">{{ $material->description }}</p>
                    @endif
                    <div class="flex items-center justify-between text-xs text-slate-400 pt-4 border-t border-slate-100">
                        ...
                        <span class="font-medium">Oleh: {{ $material->academicMap->teacher->name ?? '-' }}</span>
                    </div>
                </div>
                <div class="px-6 pb-6">
