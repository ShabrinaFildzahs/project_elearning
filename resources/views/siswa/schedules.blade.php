...
<div class="space-y-6">
    ...
    @php
        $todayId = now()->locale('id')->isoFormat('dddd');
    @endphp
    ...
    <div class="glass-card p-6 rounded-2xl flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Hari ini: <span class="font-semibold text-blue-600">{{ $todayId }}</span></p>
        </div>
    ...
    @else
        @foreach($hari_list as $hari)
            @if(isset($data_jadwal[$hari]))
                <div class="glass-card rounded-2xl overflow-hidden {{ $hari == $todayId ? 'ring-2 ring-blue-400' : '' }}">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between
                        {{ $hari == $todayId ? 'bg-blue-600' : 'bg-slate-50/50' }}">
                        <h3 class="font-bold {{ $hari == $todayId ? 'text-white' : 'text-slate-700' }}">{{ $hari }}</h3>
                        @if($hari == $todayId)
                            <span class="text-xs font-bold bg-white/20 text-white px-3 py-1 rounded-full">📍 Hari Ini</span>
                        @else
                            <span class="text-xs text-slate-400 font-medium">{{ $data_jadwal[$hari]->count() }} pelajaran</span>
                        @endif
                    </div>
                    <div class="divide-y divide-slate-100">
                        ...
                        <div class="px-6 py-4 flex items-center hover:bg-slate-50/50 transition">
                            <div class="w-28 shrink-0">
                                ...
                                <span class="text-slate-400 text-xs"> – {{ \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') }}</span>
                            </div>
                            <div class="flex-1 ml-4">
                                <p class="font-bold text-slate-800">{{ $jadwal->pemetaanAkademik->mataPelajaran->nama ?? '-' }}</p>
                                <p class="text-xs text-slate-500 mt-0.5">Kelas {{ $jadwal->pemetaanAkademik->kelas->nama ?? '-' }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-medium text-slate-600">{{ $jadwal->pemetaanAkademik->guru->nama ?? '-' }}</p>
                                @php
                                    $now = now();
