...
<div class="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            @elseif(\Carbon\Carbon::parse($tugas->tenggat_waktu)->isPast())
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-8 md:p-10">
                <div class="flex items-center gap-4 mb-6">
                    <div class="w-14 h-14 rounded-2xl bg-{{ $tugas->tipe == 'kuis' ? 'purple' : 'blue' }}-50 flex items-center justify-center text-{{ $tugas->tipe == 'kuis' ? 'purple' : 'blue' }}-600">
                        @if($tugas->tipe == 'kuis')
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black text-slate-800 leading-tight">{{ $tugas->judul }}</h1>
                        <p class="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1">Detail Tugas / Kuis</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
                    <div class="bg-slate-50/50 rounded-3xl p-6 border border-slate-100/50">
                        ...
                        <p class="text-base font-black text-slate-700">{{ $tugas->pemetaanAkademik->mataPelajaran->nama ?? '-' }}</p>
                    </div>
                    <div class="bg-slate-50/50 rounded-3xl p-6 border border-slate-100/50">
                        <div class="flex items-center gap-3 text-slate-400 mb-2">
                        ...
                        <p class="text-base font-black text-slate-700">{{ $tugas->pemetaanAkademik->kelas->nama ?? '-' }}</p>
                        <p class="text-xs font-bold text-slate-400 mt-1">{{ $tugas->pemetaanAkademik->guru->nama ?? '-' }}</p>
                    </div>
                </div>
                ...
                <div class="prose prose-slate max-w-none">
                    ...
                    <div class="text-slate-600 text-base leading-relaxed p-6 bg-slate-50/30 rounded-3xl border border-slate-100/30 whitespace-pre-line">
                        {{ $tugas->deskripsi ?? 'Tidak ada deskripsi atau instruksi khusus untuk tugas ini.' }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 group">
                ...
                @if($pengumpulan)
                    <div class="text-center py-6">
                            ...
                            <div class="relative inline-block mb-4">
                                <div class="w-24 h-24 rounded-full border-4 border-emerald-500/20 flex items-center justify-center p-2">
                                    <div class="w-full h-full rounded-full bg-emerald-500 flex flex-col items-center justify-center text-white shadow-xl shadow-emerald-200">
                                        <span class="text-3xl font-black">{{ $pengumpulan->nilai }}</span>
                                    </div>
                                </div>
                        ...
                        @else
                            <div class="w-20 h-20 bg-blue-50 rounded-[2rem] flex items-center justify-center mx-auto mb-4 animate-bounce-subtle">
                        ...
                        <div class="mt-8 p-4 bg-slate-50 rounded-3xl border border-slate-100 flex items-center gap-3 text-left">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Waktu Kirim</p>
                                <p class="text-xs font-black text-slate-700 truncate">{{ $pengumpulan->created_at->format('d M Y, H:i') }}</p>
                            </div>
                        </div>

                        @if(\Carbon\Carbon::parse($tugas->tenggat_waktu)->isFuture() && $pengumpulan->nilai === null)
                            <div class="mt-6 pt-6 border-t border-slate-100">
                                <p class="text-xs font-bold text-slate-400 mb-4 uppercase tracking-wider">Mau update jawaban?</p>
                                <form action="{{ route('siswa.assignments.submit', $tugas->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                ...
                @else
                    {{-- Active Upload Area --}}
                    <form action="{{ route('siswa.assignments.submit', $tugas->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl shadow-slate-200">
                ...
                @php
                    $tenggat = \Carbon\Carbon::parse($tugas->tenggat_waktu);
                    $diff = now()->diff($tenggat);
                    $isExpired = $tenggat->isPast();
                @endphp

                @if($isExpired)
                    <div class="text-3xl font-black text-red-400">EXPIRED</div>
                    <p class="text-xs font-bold text-slate-400 mt-2">Selesai pada {{ $tenggat->translatedFormat('d F Y, H:i') }}</p>
                @else
