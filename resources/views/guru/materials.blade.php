...
<div class="flex justify-between items-center mb-6">
    <h3 class="text-lg font-bold text-slate-800">Materi yang Diunggah <span class="text-slate-400 font-normal">({{ $data_materi->total() }})</span></h3>
    ...
    <div class="glass-card rounded-2xl p-5 hover:shadow-lg transition group">
        <div class="flex items-start justify-between mb-3">
            ...
            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                <a href="{{ route('guru.materials.edit', $materi->id) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                </a>
                <form action="{{ route('guru.materials.destroy', $materi->id) }}" method="POST" onsubmit="return confirm('Hapus materi?')">
                    @csrf @method('DELETE')
        ...
        <h4 class="font-bold text-slate-800 text-sm leading-tight">{{ $materi->judul }}</h4>
        <p class="text-xs text-slate-500 mt-1 line-clamp-2">{{ $materi->deskripsi ?: 'Tidak ada deskripsi' }}</p>
        <div class="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
            <div>
                <p class="text-xs font-semibold text-slate-700">{{ $materi->pemetaanAkademik->mataPelajaran->nama ?? '-' }}</p>
                <p class="text-xs text-slate-400">{{ $materi->pemetaanAkademik->kelas->nama ?? '-' }}</p>
            </div>
            <span class="text-xs text-slate-400">{{ $materi->created_at->diffForHumans() }}</span>
        </div>
    </div>
    @empty
...
@if($data_materi->hasPages())
<div class="mt-6">{{ $data_materi->links() }}</div>
@endif
@endsection
