...
<div class="flex justify-between items-center mb-6">
    <h3 class="text-lg font-bold text-slate-800">Daftar Tugas <span class="text-slate-400 font-normal">({{ $data_tugas->total() }})</span></h3>
    ...
    <div class="glass-card rounded-2xl p-5">
        <div class="flex items-start justify-between">
            ...
            <div class="flex items-center gap-2 shrink-0">
                <a href="{{ route('guru.assignments.submissions', $tugas->id) }}"
                   class="flex items-center space-x-1.5 px-3 py-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold rounded-lg hover:bg-emerald-100 transition">
                    ...
                    <span>{{ $tugas->pengumpulan_count }} Pengumpulan</span>
                </a>
                <a href="{{ route('guru.assignments.edit', $tugas->id) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                </a>
                <form action="{{ route('guru.assignments.destroy', $tugas->id) }}" method="POST" onsubmit="return confirm('Hapus tugas?')">
                    @csrf @method('DELETE')
...
<div class="mt-6">{{ $data_tugas->links() }}</div>
@endif
@endsection
