...
@section('page_title', 'Penilaian: ' . $assignment->title)
...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Kelas: {{ $assignment->academicMap->class->name ?? '-' }} | Mapel: {{ $assignment->academicMap->subject->name ?? '-' }}</p>
    </div>
    ...
    <table class="w-full text-left">
        <thead>
            <tr class="bg-slate-50 border-b border-slate-200">
                ...
                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Waktu Submit</th>
            ...
            <tr class="hover:bg-slate-50/50 transition">
                <td class="px-6 py-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <span class="font-semibold text-slate-800">{{ $submission->student->name ?? '-' }}</span>
                    </div>
                </td>
                <td class="px-6 py-4 text-sm">
                    @if(\Carbon\Carbon::parse($submission->created_at)->isAfter($assignment->deadline))
                        <span class="text-red-600 font-semibold px-2 py-0.5 bg-red-100 rounded-lg">Terlambat: {{ $submission->created_at->format('d M, H:i') }}</span>
                    @else
                        <span class="text-slate-600">{{ $submission->created_at->format('d M, H:i') }}</span>
                    @endif
                </td>
                <td class="px-6 py-4">
                    <a href="{{ asset('storage/' . $submission->file_path) }}" target="_blank" class="text-blue-600 hover:underline flex items-center gap-1 text-sm font-medium">
                ...
                <td class="px-6 py-4">
                    @if($submission->grade !== null)
                        <span class="text-lg font-bold {{ $submission->grade < 75 ? 'text-red-500' : 'text-emerald-600' }}">{{ $submission->grade }}</span>
                    @else
                        <span class="text-xs font-bold px-2 py-1 bg-amber-100 text-amber-700 rounded-full">Belum Dinilai</span>
                    @endif
                </td>
                <td class="px-6 py-4">
                    <form action="{{ route('guru.submissions.grade', $submission) }}" method="POST" class="flex items-center gap-2">
                        @csrf
                        <input type="number" name="grade" min="0" max="100" value="{{ $submission->grade }}" placeholder="0-100" required
